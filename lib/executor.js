...
module.exports = {
    run: function (config, job_config) {
        ...
        var transporter = nodemailer.createTransport(job_config.transport);


        return function run() {
            l.info('Running job [' + job_config.subject + ']: ' + job_config.url);

            rasterize(l, job_config.url, job_config.subject.replace(/\s/g, '_') + '.pdf', job_config.paperSize, function (filename) {
                var mailOptions;
                if (filename) {
